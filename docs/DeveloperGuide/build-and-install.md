...
Assuming that [Node.js](https://nodejs.org/) is installed.
...
> `npm ci` strictly reproduces the environment defined in `package-lock.json` and performs a clean install. This prevents dependency version mismatches between environments and ensures build reproducibility and stability.
>
> **Usage Guidelines:**
> - **`npm ci`**: Initial setup, after `package-lock.json` updates, or for a clean rebuild.
> - **`npm install`**: Only for adding new packages or upgrading existing ones.
