...
## アーキテクチャ境界におけるロギング責務 (Separation of Concerns)

アーキテクチャの階層（アプリケーション層とユーティリティ層）間における責務の分離（SoC）を徹底するため、エラーハンドリングとロギングに関して以下の原則を適用します。

1. **ユーティリティ層（`utils/`, `lib/` 等）**:
	- **`console.error` の使用を禁止**します。
	- 入力値検証の失敗や実行不可能な状態（Fail-Fast）に陥った場合は、原則として `throw` を使用して例外を上位層へ伝播させます。ログの出力可否や手段をライブラリ側で強制しません。
	- 障害調査のために内部状態の出力が必要な場合は、`console.error` ではなく `console.debug` を使用してください。外部APIの例外などを再スローする際は `{ cause: error }` で根本原因を保持します。
2. **アプリケーション層（`entrypoints/`, `background/` 等）**:
	- ユーティリティや外部APIからスローされた例外を捕捉（`try-catch`）し、システム全体への影響度を評価した上で `console.error` 等を用いて適切にログ出力を行う責務を持ちます。
