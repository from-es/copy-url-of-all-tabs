...
- **Deduplication:**
	- **Remove Duplicate URLs:** この設定を有効にすると、重複するタブ（同じURLを持つタブ）を除外して、ユニークなURLのみをコピーします。
...
- **Remove Duplicate URLs:** この設定を有効にすると、クリップボード内の重複したURLを無視し、ユニークなURLのみを新しいタブで開きます。
