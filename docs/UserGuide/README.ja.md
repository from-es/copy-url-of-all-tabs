...
> [!NOTE]
> このドキュメントは、拡張機能の**最新バージョン**を対象としています。お使いの拡張機能のバージョンによっては、一部の機能や画面表示が記載内容と異なる場合があります。

## 基本的な使い方

ツールバーにある拡張機能のアイコンをクリックすると、ポップアップメニューが表示されます。ここから基本的な操作を行います。

- **Copy (URLをコピー)**
	現在開いているすべてのタブのURLをクリップボードにコピーします。コピーする形式は設定画面で変更できます。（デフォルトはURLのみ）

- **Paste (タブを開く)**
	クリップボードにコピーされているURL群を、すべて新しいタブとして開きます。テキストの中からURLだけを自動で探し出すことも可能です。

- **Options (設定画面へ)**
	拡張機能の動作を細かくカスタマイズするための設定ページを開きます。

より詳しい動作のカスタマイズは、**Options**から開く設定ページで行うことができます。
...
> [!IMPORTANT]
> 設定を変更した後は、必ず画面左下の **Save** ボタンを押して保存してください。保存しない限り、変更は適用されません。
...
- **Tab (タブの動作)**
	...
	- **Custom Delay (カスタム遅延)**: 特定のドメインやURLパターンに対して、個別の遅延時間を設定できます。
		- `Enable (機能全体)`: このカスタム遅延機能全体を有効にします。
		- `Enable (個別ルール)`: 各カスタム遅延ルールを個別に有効/無効を切り替えます。
		- `Add`ボタンでルールを追加し、URL（前方一致）、Apply from（適用開始回数）、およびDelay（遅延時間）を設定します。リストはドラッグ＆ドロップで並べ替え可能です。
		- `Apply from (適用開始回数)`: そのルールの遅延を何回目のマッチから適用するかを指定します。例えば `1` の場合は毎回適用され、`2` の場合は2回目の一致以降から適用されます。
- **Task Control (タスク制御)**: 多数のURLを開く際の処理方法と実行順序を細かく制御します。

> [!NOTE]
> Task Control (タスク制御) については、「[上級者向け設定](#上級者向け設定)」の章をご覧ください。
...
- **Key Bindings (キー割り当て)**
	- ポップアップメニューが開いている間に、特定のキーを押すことで一時的に動作を変更する設定です。
	- **Copy Actions (コピー時の動作)**
		- `All Windows`: 指定したキーを押している間、現在のウィンドウだけでなく、開いているすべてのウィンドウのタブをコピー対象にします。
		- `Highlighted Tabs`: 指定したキーを押している間、現在選択（ハイライト）されているタブのみをコピー対象にします。
	- **Paste Actions (貼り付け時の動作)**
		- `Toggle Order (Reverse)`: 指定したキーを押している間、「クリップボードのURLリストを逆順に開く」設定を**一時的に反転**させます。
		- `Toggle Active state`: 指定したキーを押している間、「新しく開くタブをアクティブにする」設定を**一時的に反転**させます。
...
- **Enable Log Output (ログ出力の有効化)**
	- `true` の場合、ブラウザの開発者コンソールに拡張機能の動作ログを出力します。`false` の場合、ログレベルの設定に関わらず、すべてのログ出力が抑制されます。
- **Log Level (ログレベル)**
	- 出力するログの最低レベルを指定します。選択したレベル以上の重要度を持つログのみが表示されます。
		- `all`: 全てのログを出力（最も詳細）
		- `trace`: `trace` レベル以上のログを出力
		- `debug`: `debug` レベル以上のログを出力
		- `info`: `info` レベル以上のログを出力（`console.log` は `info` レベルとして扱われます）
		- `warn`: `warn` レベル以上のログを出力
		- `error`: `error` レベル以上のログを出力
		- `silent`: 全てのログを抑制
- **Add Method Label (メソッドラベルの追加)**
	- `true` の場合、各ログにメソッドの種類を示すラベル（例: `[INFO]`）を付与します。
- **Add Timestamp (タイムスタンプの追加)**
	- `true` の場合、各ログにタイムスタンプを追加します。
- **Select Timestamp Type (タイムスタンプのタイムゾーン)**
	- タイムスタンプのタイムゾーンを`UTC`または`GMT`から選択します。
...
> [!IMPORTANT]
> 少数のURL（5個程度まで）を最速で開きたい場合を除き、この組み合わせの使用は避けてください。

---

## 設定の管理

設定保存、及びファイルとして他の環境に持ち出したりできます。
