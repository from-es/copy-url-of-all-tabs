...
`freeze: true` を指定して `updateState` を呼び出すと、そのプロパティおよび配下のすべてのオブジェクトは完全に不変になります（ディープ・フリーズ）。以降、そのプロパティやネストされたプロパティへの変更はすべて拒否され、警告が出力されるか、Strict モードでは `TypeError` がスローされます。

> **ベストプラクティス:**
> `freeze: true` を指定して作成したオブジェクトの値を編集したい場合は、明示的に `cloneDeep(status.xxx)` を行い、作成した可変なコピーを編集するようにしてください。
