<script>
	...
	import { VerifyConfigValue } from "@/assets/js/lib/user/VerifyConfigValue.mjs";
	...
	async function eventSettingSave() {
		const config = cloneDeep(status.config);
	...
	async function eventSettingReset() {
		status.config = cloneDeep(status.define.Config);
	...
	async function importConfig(filetype) {
		...
		*/
		try {
			...
			const define = cloneDeep(status.define);
			const verify = new VerifyConfigValue();
			const config = verify.verify(conf, define);
	...
	function eventFormatSelectMinetype(event) {
		const minetype = event.target.value;

		status.config.Format.minetype = minetype;
	}
	// --------------------------------------------------------------------------------------------
				...
				<section class="container">
					...
					<div class="flex-main" id="Format">
						...
						<fieldset>
							...
							<select id="Format-minetype" onchange={eventFormatSelectMinetype}>
								<option value="text/plain" selected={ status.config.Format.minetype === "text/plain" ? true : false }>text/plain</option>
								<option value="text/html"  selected={ status.config.Format.minetype === "text/html"  ? true : false }>text/html</option>
							</select>
						</fieldset>
					</div>
				</section>
