 ...
 * @returns {object}
 */
export async function eventActionCopy(action, config, define) {
 ...
 * @returns {object}
 */
export async function eventActionPaste(action, config, define) {
	const regSearch = config.Search.regex;

	const status    = await ClipboardManager.readText();
	const text      = status ? status : "";
	const array     = getUrlList(text, regSearch, define);
 ...
 * @param   {boolean} regSearch - 正規表現でURLを検索するか
 * @returns {string[]}          - URLリストの配列
 */
function getUrlList(text, regSearch, define) {
	const regex = define.Regex.url.RFC3986;
	const array = regSearch ? (text).match(regex) : text.split("\n");
